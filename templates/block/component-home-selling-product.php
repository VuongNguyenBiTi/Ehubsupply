<div class="home_flashsale">
    <div class="container">
        <div class="home_flashsale_wrap">
            <div class="home_flashsale_top">
                <div class="flashsale_left">
             
                    <h2> Sản phẩm bán chạy</h2>
             

                </div>
                <div class="flashsale_right">
                    <i class="far fa-chevron-double-right"></i>
                    <a href=""> Xem tất cả</a>
                </div>
            </div>
            <div class="home_flashsale_bottom">
                <div class="product_list">
                    <div class="row">
                        <div class="col-12 col-md-6 col-lg-3 mb-3 mb-lg-1">
                            <div class="product_item">
                                <div class="product_top">
                                    <img src="https://i.imgur.com/S4S6BHY.png" alt="">
                                </div>
                                <div class="product_bottom">
                                    <h4 class="product_title">Set 4 món mỹ phẩm Eldy Blue dưỡng da trắng mọng</h4>
                                    <div class="price_wrap">
                                        <div class="oldprice">
                                            <p class="oldprice1">150,000 đ</p>
                                            <p class="oldprice2">-13%</p>
                                        </div>

                                        <div class="newprice">
                                            <p>130,000 đ</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-12 col-md-6 col-lg-3 mb-3 mb-lg-1">
                            <div class="product_item">
                                <div class="product_top">
                                    <img src="https://i.imgur.com/S4S6BHY.png" alt="">
                                </div>
                                <div class="product_bottom">
                                    <h4 class="product_title">Set 4 món mỹ phẩm Eldy Blue dưỡng da trắng mọng</h4>
                                    <div class="price_wrap">
                                        <div class="oldprice">
                                            <p class="oldprice1">150,000 đ</p>
                                            <p class="oldprice2">-13%</p>
                                        </div>

                                        <div class="newprice">
                                            <p>130,000 đ</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-12 col-md-6 col-lg-3 mb-3 mb-lg-1">
                            <div class="product_item">
                                <div class="product_top">
                                    <img src="https://i.imgur.com/S4S6BHY.png" alt="">
                                </div>
                                <div class="product_bottom">
                                    <h4 class="product_title">Set 4 món mỹ phẩm Eldy Blue dưỡng da trắng mọng</h4>
                                    <div class="price_wrap">
                                        <div class="oldprice">
                                            <p class="oldprice1">150,000 đ</p>
                                            <p class="oldprice2">-13%</p>
                                        </div>

                                        <div class="newprice">
                                            <p>130,000 đ</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-12 col-md-6 col-lg-3 mb-3 mb-lg-1">
                            <div class="product_item">
                                <div class="product_top">
                                    <img src="https://i.imgur.com/S4S6BHY.png" alt="">
                                </div>
                                <div class="product_bottom">
                                    <h4 class="product_title">Set 4 món mỹ phẩm Eldy Blue dưỡng da trắng mọng</h4>
                                    <div class="price_wrap">
                                        <div class="oldprice">
                                            <p class="oldprice1">150,000 đ</p>
                                            <p class="oldprice2">-13%</p>
                                        </div>

                                        <div class="newprice">
                                            <p>130,000 đ</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                
                    </div>

                </div>
            </div>
        </div>
    </div>
</div>

